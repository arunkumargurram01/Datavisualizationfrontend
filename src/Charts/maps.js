/* const cdata = [
    ...
    const CustomersOnMap = ({ customerData }) => {
      const [mapsData, setMapsData] = useState([]);
      const centerPosition = [37.0902, -95.7129]; // Center of the map (e.g., coordinates of USA)
    
    /*   useEffect(() => {
        if (customerData && customerData.length > 0) {
          setMapsData(customerData);
        }
      }, [customerData]); */
      ...
      return (
        <MapContainer center={centerPosition} zoom={4} style={{ height: "500px", width: "100%" }}>
          <TileLayer
            url="https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png"
            attribution='&copy; <a href="https://www.openstreetmap.org/copyright">OpenStreetMap</a> contributors'
          />
          {mapsData && mapsData.length > 0 ? (
            mapsData.map((city, index) => (
